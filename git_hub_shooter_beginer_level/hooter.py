...
#leod music and sound
pygame.mixer.music.load('sound/audio_music2.mp3')
pygame.mixer.music.set_volume(0.2)
pygame.mixer.music.play(-1, 0.0, 10000)

jump_fx = pygame.mixer.Sound('sound/audio_jump.wav')
jump_fx.set_volume(0.15)
shot_fx = pygame.mixer.Sound('sound/audio_shot.wav')
shot_fx.set_volume(0.15)
grenade_fx = pygame.mixer.Sound('sound/audio_grenade.wav')
...
# background images
victory_img = pygame.image.load('image/background/victory_bg.png').convert_alpha()
...
for x in range(TILE_TYPES):
    img = pygame.image.load(f'image/title/{x}.png').convert_alpha()
...
def draw_bg():
    ...
    for x in range(5):
        screen.blit(cloud_img, ((x * width) - bg_scroll * 0.5, 0))
        screen.blit(mountain_img, ((x * width) - bg_scroll * 0.6, SCREEN_HEIGHT - mountain_img.get_height() - 300))
        screen.blit(pine1_img, ((x * width) - bg_scroll * 0.7, SCREEN_HEIGHT - pine1_img.get_height() - 150))
        screen.blit(pine2_img, ((x * width) - bg_scroll * 0.8, SCREEN_HEIGHT - pine2_img.get_height()))

# function to reset to level
def reset_level():
    enemy_group.empty()
    bullet_group.empty()
    grenade_group.empty()
...
class Soldier(pygame.sprite.Sprite):
    def __init__(self, char_type, x, y, scale, speed, ammo, grenades):
        ...
        for animation in animation_types:
            ...
            temp_list = []
            # current number of files in folder
        ...
        self.rect = self.image.get_rect()  # drawing rect around img to get collisions and pos
    ...
    def move(self, moving_left, moving_right):
        ...
        # check for collision with victory
        victory = False
        if pygame.sprite.spritecollide(self, victory_group, False):
            victory = True
        ...
        return screen_scroll, level_complete, victory
    ...
    def ai(self):
        if self.alive and player.alive:
            if self.idling == False and random.randint(1, 300) == 1:
                ...
                self.idling = True
                self.idling_counter = random.randint(50, 130)
    ...
    def update_animation(self):
        ...
        # update image depending on current image
        self.image = self.animation_list[self.action][self.frame_index]
        ...
        # if the animation has run out then reset to the back to start
    ...
    def draw(self):
        screen.blit(pygame.transform.flip(self.image, self.flip, False), self.rect)  # to img = shape,, rect = pos
...
class World():
    ...
    def process_data(self, data):
        ...
        #iterate through each value in level data tile
            ...
            for x, tile in enumerate(row):
                if tile >= 0:
                    ...
                    if tile >= 0 and tile <= 8:
                        ...
                        player = Soldier('soldier', x * TILE_SIZE, y * TILE_SIZE, 1.5, 5, 20, 5)
                        ...
                        exit_group.add(exit)
                    elif tile == 21:# create victory
                        victory = Victory(img, x * TILE_SIZE, y * TILE_SIZE)
                        victory_group.add(victory)
...
class Item_box(pygame.sprite.Sprite):
    ...
    def update(self):
        # scroll
        self.rect.x += screen_scroll
        # check the player has picked up the box
...
class Grenade(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        for tile in world.obstacle_list:
            # check if grenade is gone off the walls
        ...
        if self.timer <= 0:
            ...
            if abs(self.rect.centerx - player.rect.centerx) < TILE_SIZE * 2 and \
                    abs(self.rect.centerx - player.rect.centerx) < TILE_SIZE * 3:
                player.health -= 40
            for enemy in enemy_group:
                if abs(self.rect.centerx - enemy.rect.centerx) < TILE_SIZE * 2 and \
                        abs(self.rect.centerx - enemy.rect.centerx) < TILE_SIZE * 3:
                    enemy.health -= 50
...
class Explosion(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.counter >= EXPLOSION_SPEED:
            self.counter = 0
            self.frame_index += 1
            # if the animation complete then delete the explosion
...
class ScreenFade():
    ...
    def fade(self):
        ...
        if self.direction == 3: # whole screen fade return
            pygame.draw.rect(screen, self.color, (0, 0, 0 + self.fade_counter, SCREEN_HEIGHT))
            pygame.draw.rect(screen, self.color, (0, 0, SCREEN_WIDTH, 0 + self.fade_counter))
...
victory_fade = ScreenFade(3, VIC, 4)
...
exit1_button = button.Button(SCREEN_WIDTH // 2 + 145, SCREEN_HEIGHT // 2 + 170, exit_img, 0.5)
restart1_button = button.Button(SCREEN_WIDTH // 2 - 300, SCREEN_HEIGHT // 2 + 170, restart_img, 1.5)
    ...
    if victory == True:
        ...
        if victory_fade.fade():
            ...
            if exit1_button.draw(screen):
                run = False
                victory = False
            if restart1_button.draw(screen):
        ...
        if start_game == False:
            ...
            for x in range(player.ammo):
                screen.blit(bullet_img, (95 + (x * 10), 40))
            # show grenade
            draw_text(f'Grenade : ', font, WHITE, 10, 60)
            for x in range(player.grenades):
                screen.blit(grenade_img, (125 + (x * 15), 65))
            ...
            draw_text(f'Kill count : {enemy_kill}', font, WHITE, 10, 85)
            ...
            if player.alive:
                ...
                screen_scroll, level_complete, victory = player.move(moving_left, moving_right)
